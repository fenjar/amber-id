#!/bin/bash
#SBATCH --job-name=avatarfp-stgcn-tavd-g1-head-ce
...
SCRATCH=/netscratch/$USER/tavd_g1_ce
...
srun -K \
  ...
  --container-mounts="$WORKDIR":"$WORKDIR",$DATADIR:$DATADIR:ro,$SCRATCH:$SCRATCH \
    python stgcndual.py \
    --data_root /netscratch/fschulz/g1_train_landmarks_npy \
    --save_dir /netscratch/fschulz/tavd_g1_ce \
    --num_epochs 3000 \
    --warmup_epochs 3000 \
    --embedding_size 256 \
    --amp \
