#!/bin/bash
#SBATCH --job-name=avatarfp-stgcn-tavd-k3
...
SCRATCH=/netscratch/$USER/tavd_k3_ce
...
srun -K \
  ...
  --container-mounts="$WORKDIR":"$WORKDIR",$DATADIR:$DATADIR:ro,$SCRATCH:$SCRATCH \
    python stgcndual.py \
    --data_root /netscratch/fschulz/cv_folds/k3_train_landmarks_npy \
    --save_dir /netscratch/fschulz/tavd_k3_ce \
    --num_epochs 3000 \
    --warmup_epochs 3000 \
    --embedding_size 256 \
    --amp \
